...
def entrypoint(argv : List[str])->None:
    ...
    parser.add_argument("-p","--paths",help="filters the repositories by path", nargs="*",type=str)
    parser.add_argument("-a","--authors",help="filters the repositories by author", nargs="*",type=str)
    parser.add_argument("-t","--tags",help="filters the repositories by tags", nargs="*",type=str)
    parser.add_argument("-o","--origins",help="filters the repositories by origins", nargs="*",type=str)
    
    # parse argv
    args = parser.parse_args(argv)
    
    # fetch repositories data
    repositories = core.repositories.get_repositories_filtered(args.authors, args.paths, args.origins, args.tags)
